...
interface CharacterFilePayload {
  ...
  interactive_sections?: Array<{ title?: string; type?: string; button_text?: string; lines?: string[]; images?: string[] }>
  images?: {
    urls?: string[]
...
async function resolveDataPaths(): Promise<DataPaths> {
  ...
  for (const projectRoot of projectRootCandidates) {
    ...
    const charactersDir = resolve(dataDir, 'characters')
    if (await isDirectory(charactersDir)) {
      return {
        projectRoot,
        dataDir,
        charactersDir,
  ...
  const fallbackRoot = projectRootCandidates[0]
  return {
    projectRoot: fallbackRoot,
    dataDir: resolve(fallbackRoot, 'data'),
    charactersDir: resolve(fallbackRoot, 'data', 'characters'),
    imagesDir: resolve(fallbackRoot, 'data', 'images')
  }
}

function getImageDir(imagesDir: string, name: string): string {
  return resolve(imagesDir, sanitizeName(name))
}

async function readCharacterFileNames(): Promise<string[]> {
  const paths = await resolveDataPaths()
  if (!(await isDirectory(paths.charactersDir))) {
    return []
  }

  const entries = await readdir(paths.charactersDir, { withFileTypes: true })
  return entries
...
async function readCharacterPayload(
  fileName: string
): Promise<{ fileName: string; jsonPath: string; rawJson: string; payload: CharacterFilePayload; paths: DataPaths }> {
  const paths = await resolveDataPaths()
  const jsonPath = resolve(paths.charactersDir, fileName)
  const rawJson = await readFile(jsonPath, 'utf-8')
  const payload = JSON.parse(rawJson) as CharacterFilePayload
  return { fileName, jsonPath, rawJson, payload, paths }
}

function normalizeDownloadedImages(
  downloaded: DownloadedCharacterImage[] | undefined,
  projectRoot: string
): Promise<DownloadedCharacterImage[]> {
  return Promise.all(
    (downloaded ?? []).map(async (image) => {
    const absolutePath = isAbsolute(image.local_path) ? image.local_path : resolve(projectRoot, image.local_path)
...
function toSummary(fileName: string, jsonPath: string, payload: CharacterFilePayload, paths: DataPaths): CharacterSummary {
  ...
  return {
    ...
    jsonPath,
    imageDir: getImageDir(paths.imagesDir, name)
  }
}
...
async function toDetail(
  ...
  return {
    ...
    images: {
      ...
      downloaded: await normalizeDownloadedImages(payload.images?.downloaded, paths.projectRoot),
...
export async function listCharacters(): Promise<CharacterSummary[]> {
  ...
  for (const fileName of fileNames) {
    try {
      const { jsonPath, payload, paths } = await readCharacterPayload(fileName)
      summaries.push(toSummary(fileName, jsonPath, payload, paths))
    } catch (error) {
...
export async function getCharacterDetail(identifier: string): Promise<CharacterDetail> {
  ...
  if (fileNames.includes(directFileName)) {
    const { fileName, jsonPath, rawJson, payload, paths } = await readCharacterPayload(directFileName)
    return toDetail(fileName, jsonPath, rawJson, payload, paths)
  }

  for (const fileName of fileNames) {
    try {
      const { jsonPath, rawJson, payload, paths } = await readCharacterPayload(fileName)
